...
# Parallel iCF pipeline — Step 3b: Merge batches + finalize
#
# Loads all batch outputs from 02c1_icf_step3_batch.R, concatenates
# results, performs plurality voting, builds synthetic tree, and
# calculates subgroup CATEs.
#
# Usage: Rscript 02c2_icf_step3_merge.R
...
here::i_am("suicidality/analysis-icf/02c2_icf_step3_merge.R")
...
for (d_idx in seq_along(depths)) {
  ...
  for (fold in 1:K) {
    ...
    if (file.exists(cv_file)) {
      ...
      n_cv_missing <- n_cv_missing + 1L
    }
  }
}

if (n_cv_missing > 0) {
  pct <- round(100 * n_cv_missing / n_cv_expected, 1)
  warning(sprintf("%d of %d CV files missing (%s%%) — their MSE set to Inf. Depth selection may be unreliable.",
                  n_cv_missing, n_cv_expected, pct))
}
...
results <- list(
  ...
  # after intermediate-file cleanup. cf_raw is intentionally excluded
  # (it's already null and was the largest object).
  X = step1$X,
  Y = step1$Y,
  W = step1$W,
